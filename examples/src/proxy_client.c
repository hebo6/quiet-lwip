...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0) {
            printf("Usage: %s <server_addr> [--profile <name>]\n\n"
                   "Arguments:\n"
                   "  server_addr       remote proxy server address\n\n"
                   "Options:\n"
    ...
    conf->encoder_opt =
        quiet_encoder_profile_filename(fname, profile_key);
    if (!conf->encoder_opt) {
    ...
    conf->decoder_opt =
        quiet_decoder_profile_filename(fname, profile_key);
    if (!conf->decoder_opt) {
    ...
    for (;;) {
        ...
        int remote_fd = open_send(server_addr);
